    ...
    .map((node, index) => {
      const safePosition =
        ...
        typeof node.position.y === 'number'
          ? node.position
          : { x: (index % 5) * 250, y: Math.floor(index / 5) * 180 }

      return {
        id: String(node.id),
        data: { label: node.data?.label || node.label || node.id },
  ...
  if (!nodes.length) {
    return (
      <div className="arch-empty">
        ...
        <span>This repository may have very few internal imports.</span>
      </div>
    )
  }
    ...
    <div className="arch-wrapper">
      <div className="arch-header">
        <div className="arch-stats">
          <span>Nodes: {archGraph.totalNodes || 0}</span>
          <span>Edges: {archGraph.totalEdges || 0}</span>
          <button
            className={`arch-lock-btn ${locked ? 'locked' : ''}`}
